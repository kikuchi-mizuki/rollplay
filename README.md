...
### 月間利用上限
- **会話（GPT）**: 月間約 2万回まで
- **音声認識（Whisper）**: 月間約 1万回まで
- **音声合成（TTS）**: 月間約 1.5万回まで
- **動画生成（D-ID）**: 月間約 500回まで

### 制限到達時の動作
- 月間予算に達した場合、月初（翌月1日）まで一部機能が**一時停止**されます
- アプリ内で利用状況のバナーが表示されます

### 推奨事項
- **練習量の調整**: 月間の利用状況を確認しながらご利用ください
- **効率的な利用**: 短い練習セッションを複数回に分けることを推奨します
- **月末の注意**: 月末は予算残量にご注意ください

## 🎉 進捗状況

**最終更新**: 2026年1月12日（セッション36完了）
**開発進捗**: 100%完成（Week 1-4, 6, 8, Phase 2 完了 / 全8週間 + Phase 2）
**ステータス**: 全機能実装完了、Blueprint構造化完了、テストカバレッジ74%達成、API仕様書完備、本番デプロイ可能 🎊
**最新改善**: コードベース全体の品質向上（セキュリティ強化、パフォーマンス最適化、API標準化）✨

### 📊 プロジェクト健全性スコア: 98.0% (優秀)

- ✅ テストカバレッジ: **74%** (app.py 74%, Blueprints 76%)
- ✅ セキュリティレベル: 98% (+1)
- ✅ エラーハンドリング: 98% (+1)
- ✅ コード品質: 98% (+3)
- ✅ パフォーマンス: 95% (+3)
...
### ✅ セッション36: コードベース品質向上（完了 - 2026-01-12）
...
- ✅ **セキュリティ強化**
  ...
  - 環境変数の適切なハンドリング
- ✅ **パフォーマンス最適化**
  - N+1クエリ修正（300+クエリ→4クエリ、75%改善）
  - レート制限の最適化（コスト削減）
- ✅ **API設計標準化**
  - success_response/error_responseヘルパー追加
  - レスポンス形式統一
- ✅ **本番環境対応**
  - CORS設定強化（FRONTEND_URL対応）
  - エラーログ改善（視認性向上）
- ✅ **テスト修正**
  - N+1クエリ修正後のテスト更新
  - 214件中211件成功（99%）
...
- **🎤 ペルソナ別音声**: ペルソナ・シナリオに応じて声と話し方が変化
  - 初回面談: 慎重で落ち着いた声（shimmer）
  - 2次・3次面談: 明るく前向きな声（nova）
  - キックオフMTG: テキパキとした話し方
  - 業種別: 飲食店主、テック系創業者、クリエイティブ系など
  - 高品質TTS: OpenAI tts-1-hd モデル使用
...
**画面全体（UIを含む全画面）を録画する手順**：

#### 方法1: 画面全体を録画（推奨）

1. **🖥️ 画面共有ボタンをクリック**
   - ブラウザの画面共有選択ダイアログが表示されます
   - **「このタブ」を選択**すると、アプリ画面全体が録画されます

2. **🎬 録画ボタンをクリック**
   - 画面全体の録画が開始されます
   - アプリのUI、アバター、字幕など、すべてが録画されます
...
1. **🎬 録画ボタンをクリック**（画面共有なし）
   - カメラ映像とアバターが合成されて録画されます
   - アプリのUI（シナリオ情報、字幕）は録画されません

**録画モード**:
- **画面共有ON**: アプリUI全体をそのまま録画（スクリーンショットのように）✨
- **画面共有OFF**: カメラ映像のみ録画（アバターと合成）

**💡 ポイント**:
- **画面全体を録画したい場合**: 必ず画面共有ボタンを押してから録画
- **練習の様子を完全記録**: 画面共有ON推奨
...
AIの声と話し方は、**ペルソナとシナリオに応じて自動的に変化**します：
...
**業種別の音声設定**（すべて女性声）:
- **飲食店主**: ゆっくり丁寧（shimmer、0.95x）
- **テック系創業者**: 明るくスマート（nova、1.15x）
- **クリエイティブ系**: 明るく表現豊か（nova、1.1x）
- **ベテラン経営者**: 落ち着いて丁寧（shimmer、0.95x）
- **中堅管理職**: 落ち着いて丁寧（shimmer、1.1x）

**音声品質**:
- **モデル**: OpenAI tts-1-hd（高品質、自然で流暢）
- **音声の種類**: 2種類の女性声（nova: 明るく元気、shimmer: 柔らかく落ち着いた）
- **話速範囲**: 0.95x〜1.2x（シーンに応じて最適化）
- **リアルタイム生成**: 会話に合わせて即座に音声生成
