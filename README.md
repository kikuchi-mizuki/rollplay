...
**録画機能の特徴（セッション40で最終修正完了）:**
- 3つの録画モード（画面共有のみ、画面共有+カメラ、カメラのみ）
- **ユーザーの声とAIの応答の両方を確実に録音** ✅（直接接続方式で安定化）
- **アバターの表情変化も録画に反映**（thinking→listening等）✅
- 高品質な動画（VP9+Opus、2.5Mbps）
- カメラのみモード: カメラ映像+アバターを1920x1080で合成
- Web Audio APIの直接接続によるAI音声キャプチャ（AudioBufferSource→destination & MediaStreamDestination）
