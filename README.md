...
**録画モード**:
- **画面共有のみ**: アプリUI全体をそのまま録画（カメラなし）
- **画面共有+カメラ**: アプリUI + カメラ（右下PinP）を合成録画

**なぜ画面共有が必要？**
- 画面共有なしでは、カメラ映像のみが録画されます
- アプリのUI（シナリオ情報、字幕、アバターなど）を含めるには画面共有が必須です
- 練習の様子を完全に記録するため、必ず画面共有をONにしてください
