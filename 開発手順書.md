# 📋 SNS動画営業ロープレシステム 開発手順書

> 要件定義書v4.0に基づく8週間の詳細実装ガイド

---

## 📍 現在地

```
開発進捗: 35%完成（Week 1完了 / 全8週間）

■■■■■■■■□□□□□□□□□□□□□□□□

✅ 完了: 音声対話の基本機能（STEP 0-4相当）
✅ 完了: RAGの基本構造（STEP 6の一部）
✅ 完了: Week 1 - Google認証 + 店舗管理基盤（2025-11-11）
❌ 未着手: Week 2以降（6シナリオ構築〜デプロイ）

次のステップ: Week 2 - 6シナリオ構築 + 音声データ処理
```

---

## 🗓️ 8週間スケジュール概要

| Week | フェーズ | 主要成果物 | 進捗 |
|------|---------|-----------|------|
| **Week 1** | Google認証 + 店舗管理基盤 | 認証付きアプリ | ✅ **完了（2025-11-11）** |
| **Week 2** | 6シナリオ + RAG構築 | RAGデータ600-800件 | ⏳ 次のステップ |
| **Week 3** | データ永続化 | Supabase統合完了 | ⬜ 未着手 |
| **Week 4** | 履歴閲覧 + CSV出力 | 成長可視化機能 | ⬜ 未着手 |
| **Week 5** | 評価精度向上 | 講師評価80%一致 | ⬜ 未着手 |
| **Week 6** | 店舗管理機能 | フランチャイズ管理 | ⬜ 未着手 |
| **Week 7** | パフォーマンス最適化 | 100店舗対応完了 | ⬜ 未着手 |
| **Week 8** | テスト・デプロイ | 本番稼働可能 | ⬜ 未着手 |

---

## 📅 Week 1: Google認証 + 店舗管理基盤

**目標**: 認証付きアプリ（店舗コード必須）

### ✅ **完了状況（2025-11-11）**

**実装完了した機能:**
- ✅ Supabaseプロジェクト作成・環境変数設定
- ✅ Google OAuth認証（ログイン・ログアウト）
- ✅ 4テーブル作成（stores, profiles, conversations, evaluations）
- ✅ RLS（Row Level Security）設定
- ✅ 初回登録フロー（店舗コード検証 → プロフィール作成）
- ✅ 本部管理者専用の店舗管理画面（店舗コード自動生成・一覧表示）
- ✅ 認証状態管理（AuthContext + React Router）
- ✅ ダークテーマUI統一（全認証画面 + 管理画面）

**発生した問題と解決:**
1. **RLS循環参照問題** - 管理者権限チェックがprofilesテーブルを参照し無限ループ
   - 解決: ポリシーを簡素化し、認証ユーザー全員が閲覧可能に変更
2. **認証初期化の競合状態** - `initAuth`と`onAuthStateChange`が競合し読み込み無限ループ
   - 解決: `isInitialized`フラグを導入し、初期化完了後のみイベント処理
3. **UI不統一** - ログイン/登録画面が旧デザイン
   - 解決: 全画面を暗いグラデーション背景 + ガラスカードに統一

**実装ファイル:**
- `src/lib/supabase.ts` - Supabaseクライアント
- `src/contexts/AuthContext.tsx` - 認証状態管理
- `src/components/Auth/LoginPage.tsx` - ログイン画面
- `src/components/Auth/RegisterPage.tsx` - 初回登録画面
- `src/components/Auth/AuthCallback.tsx` - OAuth コールバック
- `src/components/Admin/StoreManagementPage.tsx` - 店舗管理画面
- `src/components/Header.tsx` - ヘッダー（ログアウト機能）
- `database/01_create_stores.sql` 〜 `06_fix_rls_circular_reference.sql` - データベースマイグレーション

---

### Day 1: Supabaseプロジェクト作成

#### タスク

- [ ] Supabaseアカウント作成（https://supabase.com）
- [ ] 新規プロジェクト作成
  - プロジェクト名: `rollplay-sns-video`
  - リージョン: `Tokyo (Northeast Asia)`
  - プラン: Free（開発用）→ Pro（本番用）
- [ ] プロジェクトURL・API Keyを取得
  - Project URL: `https://xxx.supabase.co`
  - anon key: `eyJhbGc...`
  - service_role key: `eyJhbGc...`（管理用）

#### 設定ファイル更新

```bash
# .envファイルに追加
VITE_SUPABASE_URL=https://xxx.supabase.co
VITE_SUPABASE_ANON_KEY=eyJhbGc...
SUPABASE_SERVICE_ROLE_KEY=eyJhbGc...
```

#### チェックリスト

- [ ] Supabaseプロジェクト作成完了
- [ ] .envファイルに認証情報追加
- [ ] Supabase Dashboardにアクセス可能

---

### Day 2: Google OAuth設定

#### タスク1: Google Cloud Console設定

- [ ] Google Cloud Console にアクセス（https://console.cloud.google.com）
- [ ] 新規プロジェクト作成: `rollplay-sns-video`
- [ ] OAuth同意画面を設定
  - User Type: 外部
  - アプリ名: `SNS動画営業ロープレシステム`
  - サポートメール: （自社メール）
- [ ] 認証情報 → OAuth 2.0 クライアントID作成
  - アプリケーションの種類: ウェブアプリケーション
  - 承認済みのリダイレクトURI:
    ```
    https://xxx.supabase.co/auth/v1/callback
    ```
- [ ] クライアントID・シークレットを取得
  - クライアントID: `123456789-xxx.apps.googleusercontent.com`
  - クライアントシークレット: `GOCSPX-xxx`

#### タスク2: Supabase Auth設定

- [ ] Supabase Dashboard → Authentication → Providers
- [ ] Google を有効化
- [ ] Google Client ID・Secret を入力
- [ ] Redirect URLを確認

#### チェックリスト

- [ ] Google OAuth設定完了
- [ ] Supabase側でGoogle認証有効化
- [ ] テスト用Googleアカウントでログイン確認

---

### Day 3: データベース設計・構築

#### タスク1: stores テーブル作成

```sql
-- Supabase SQL Editor で実行

-- 店舗マスタ
CREATE TABLE stores (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  store_code TEXT UNIQUE NOT NULL,
  store_name TEXT NOT NULL,
  region TEXT,
  status TEXT DEFAULT 'active',
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

-- インデックス
CREATE INDEX idx_store_code ON stores(store_code);
CREATE INDEX idx_region ON stores(region);
CREATE INDEX idx_status ON stores(status);

-- サンプルデータ挿入
INSERT INTO stores (store_code, store_name, region, status) VALUES
  ('STORE_001', '東京銀座店', '関東', 'active'),
  ('STORE_002', '大阪梅田店', '関西', 'active'),
  ('STORE_003', '福岡天神店', '九州', 'active');
```

#### タスク2: profiles テーブル作成

```sql
-- ユーザープロフィール（Supabase Authと連携）
CREATE TABLE profiles (
  id UUID PRIMARY KEY REFERENCES auth.users ON DELETE CASCADE,
  store_id UUID REFERENCES stores ON DELETE SET NULL,
  store_code TEXT,
  display_name TEXT NOT NULL,
  email TEXT UNIQUE NOT NULL,
  avatar_url TEXT,
  role TEXT DEFAULT 'user',  -- admin/manager/user
  created_at TIMESTAMP DEFAULT NOW(),
  last_active_at TIMESTAMP
);

-- インデックス
CREATE INDEX idx_profile_store ON profiles(store_id);
CREATE INDEX idx_profile_email ON profiles(email);
CREATE INDEX idx_profile_role ON profiles(role);
```

#### タスク3: conversations テーブル作成

```sql
-- 会話履歴
CREATE TABLE conversations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES profiles ON DELETE CASCADE,
  store_id UUID REFERENCES stores,
  scenario_id TEXT NOT NULL,
  scenario_title TEXT,
  messages JSONB NOT NULL,
  duration_seconds INT,
  created_at TIMESTAMP DEFAULT NOW()
);

-- インデックス
CREATE INDEX idx_conv_user ON conversations(user_id, created_at DESC);
CREATE INDEX idx_conv_store ON conversations(store_id, created_at DESC);
CREATE INDEX idx_conv_scenario ON conversations(scenario_id);
```

#### タスク4: evaluations テーブル作成

```sql
-- 評価履歴
CREATE TABLE evaluations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id UUID REFERENCES conversations ON DELETE CASCADE,
  user_id UUID REFERENCES profiles ON DELETE CASCADE,
  store_id UUID REFERENCES stores,
  scenario_id TEXT NOT NULL,
  scores JSONB NOT NULL,
  total_score INT,
  average_score DECIMAL(3,2),
  comments JSONB,
  created_at TIMESTAMP DEFAULT NOW()
);

-- インデックス
CREATE INDEX idx_eval_user ON evaluations(user_id, created_at DESC);
CREATE INDEX idx_eval_store ON evaluations(store_id, created_at DESC);
CREATE INDEX idx_eval_scenario ON evaluations(scenario_id);
```

#### チェックリスト

- [ ] 全テーブル作成完了
- [ ] サンプルデータ挿入確認
- [ ] Supabase Table Editorで確認

---

### Day 4: RLS（Row Level Security）設定

#### タスク1: conversations用RLS

```sql
-- RLSを有効化
ALTER TABLE conversations ENABLE ROW LEVEL SECURITY;

-- ユーザーは自分のデータのみ閲覧
CREATE POLICY "Users can view own conversations"
  ON conversations FOR SELECT
  USING (auth.uid() = user_id);

-- ユーザーは自分のデータを作成
CREATE POLICY "Users can insert own conversations"
  ON conversations FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- 店舗管理者は自店舗のデータを閲覧
CREATE POLICY "Managers can view store conversations"
  ON conversations FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid()
        AND role = 'manager'
        AND store_id = conversations.store_id
    )
  );

-- 本部管理者は全データ閲覧
CREATE POLICY "Admins can view all conversations"
  ON conversations FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
    )
  );
```

#### タスク2: evaluations用RLS

```sql
-- RLSを有効化
ALTER TABLE evaluations ENABLE ROW LEVEL SECURITY;

-- 同様のポリシーを設定
CREATE POLICY "Users can view own evaluations"
  ON evaluations FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own evaluations"
  ON evaluations FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Managers can view store evaluations"
  ON evaluations FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid()
        AND role = 'manager'
        AND store_id = evaluations.store_id
    )
  );

CREATE POLICY "Admins can view all evaluations"
  ON evaluations FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
    )
  );
```

#### チェックリスト

- [ ] RLS設定完了
- [ ] ポリシーテスト（一般ユーザーで確認）
- [ ] 権限漏洩がないことを確認

---

### Day 5: フロントエンド - Supabase統合

#### タスク1: Supabaseクライアント設定

```bash
# パッケージインストール
npm install @supabase/supabase-js
```

```typescript
// src/lib/supabase.ts
import { createClient } from '@supabase/supabase-js'

const supabaseUrl = import.meta.env.VITE_SUPABASE_URL
const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY

export const supabase = createClient(supabaseUrl, supabaseAnonKey)
```

#### タスク2: Google認証UI

```tsx
// src/components/Auth/LoginPage.tsx
import { supabase } from '@/lib/supabase'

export function LoginPage() {
  const handleGoogleLogin = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback`
      }
    })
    if (error) console.error('Login error:', error)
  }

  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="max-w-md w-full p-8 bg-white rounded-lg shadow-lg">
        <h1 className="text-2xl font-bold mb-6 text-center">
          SNS動画営業ロープレシステム
        </h1>

        <button
          onClick={handleGoogleLogin}
          className="w-full bg-white border border-gray-300 rounded-lg p-3 flex items-center justify-center gap-3 hover:bg-gray-50"
        >
          <img src="/google-icon.svg" alt="Google" className="w-5 h-5" />
          Googleでログイン
        </button>
      </div>
    </div>
  )
}
```

#### タスク3: 初回登録フロー（店舗コード入力）

```tsx
// src/components/Auth/RegisterPage.tsx
import { useState } from 'react'
import { supabase } from '@/lib/supabase'

export function RegisterPage() {
  const [storeCode, setStoreCode] = useState('')
  const [storeCodeValid, setStoreCodeValid] = useState(false)
  const [storeName, setStoreName] = useState('')
  const [displayName, setDisplayName] = useState('')

  const user = supabase.auth.getUser()

  // 店舗コード検証
  const verifyStoreCode = async (code: string) => {
    const { data, error } = await supabase
      .from('stores')
      .select('*')
      .eq('store_code', code)
      .eq('status', 'active')
      .single()

    if (data && !error) {
      setStoreCodeValid(true)
      setStoreName(data.store_name)
    } else {
      setStoreCodeValid(false)
      setStoreName('')
    }
  }

  const handleRegister = async () => {
    // profilesテーブルに登録
    const { error } = await supabase
      .from('profiles')
      .insert({
        id: user.id,
        store_code: storeCode,
        display_name: displayName,
        email: user.email,
        avatar_url: user.user_metadata.avatar_url
      })

    if (!error) {
      // ホーム画面へリダイレクト
      window.location.href = '/'
    }
  }

  return (
    <div className="max-w-md mx-auto p-8">
      <h2 className="text-2xl font-bold mb-6">アカウント登録</h2>

      {/* Googleから取得した情報 */}
      <div className="bg-gray-100 p-4 rounded mb-4">
        <img src={user.user_metadata.avatar_url} className="w-16 h-16 rounded-full" />
        <p className="font-semibold">{user.user_metadata.full_name}</p>
        <p className="text-sm text-gray-600">{user.email}</p>
      </div>

      {/* 店舗コード入力 */}
      <div className="mb-4">
        <label className="block text-sm font-medium mb-2">
          店舗コード（必須）
        </label>
        <input
          type="text"
          placeholder="例: STORE_001"
          value={storeCode}
          onChange={(e) => {
            setStoreCode(e.target.value)
            verifyStoreCode(e.target.value)
          }}
          className="w-full p-2 border rounded"
        />
        {storeCodeValid && (
          <p className="text-green-500 text-sm mt-1">
            ✅ {storeName} として登録されます
          </p>
        )}
        {storeCode && !storeCodeValid && (
          <p className="text-red-500 text-sm mt-1">
            ⚠️ 店舗コードが見つかりません
          </p>
        )}
      </div>

      {/* 表示名 */}
      <div className="mb-4">
        <label className="block text-sm font-medium mb-2">表示名</label>
        <input
          type="text"
          value={displayName}
          onChange={(e) => setDisplayName(e.target.value)}
          className="w-full p-2 border rounded"
        />
      </div>

      <button
        onClick={handleRegister}
        disabled={!storeCodeValid || !displayName}
        className="w-full bg-primary text-white p-3 rounded disabled:opacity-50"
      >
        登録完了
      </button>
    </div>
  )
}
```

#### チェックリスト

- [ ] Supabaseクライアント設定完了
- [ ] Googleログイン動作確認
- [ ] 店舗コード検証動作確認
- [ ] 初回登録フロー完了

---

### Day 6: 本部管理者 - 店舗コード発行UI

#### タスク1: 管理者ページ作成

```tsx
// src/components/Admin/StoreManagement.tsx
import { useState, useEffect } from 'react'
import { supabase } from '@/lib/supabase'

export function StoreManagement() {
  const [stores, setStores] = useState([])
  const [newStore, setNewStore] = useState({
    store_code: '',
    store_name: '',
    region: ''
  })

  // 店舗一覧取得
  useEffect(() => {
    fetchStores()
  }, [])

  const fetchStores = async () => {
    const { data } = await supabase
      .from('stores')
      .select('*')
      .order('store_code')
    setStores(data || [])
  }

  // 新規店舗追加
  const handleAddStore = async () => {
    const { error } = await supabase
      .from('stores')
      .insert(newStore)

    if (!error) {
      setNewStore({ store_code: '', store_name: '', region: '' })
      fetchStores()
    }
  }

  // 次の店舗コード生成
  const generateNextCode = () => {
    const lastStore = stores[stores.length - 1]
    if (!lastStore) return 'STORE_001'

    const lastNumber = parseInt(lastStore.store_code.split('_')[1])
    const nextNumber = (lastNumber + 1).toString().padStart(3, '0')
    return `STORE_${nextNumber}`
  }

  return (
    <div className="max-w-4xl mx-auto p-8">
      <h2 className="text-2xl font-bold mb-6">店舗管理</h2>

      {/* 新規店舗追加 */}
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        <h3 className="text-lg font-semibold mb-4">店舗コード発行</h3>

        <div className="grid grid-cols-3 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium mb-1">店舗コード</label>
            <input
              type="text"
              value={newStore.store_code}
              onChange={(e) => setNewStore({...newStore, store_code: e.target.value})}
              placeholder={generateNextCode()}
              className="w-full p-2 border rounded"
            />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">店舗名</label>
            <input
              type="text"
              value={newStore.store_name}
              onChange={(e) => setNewStore({...newStore, store_name: e.target.value})}
              placeholder="例: 東京銀座店"
              className="w-full p-2 border rounded"
            />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">リージョン</label>
            <select
              value={newStore.region}
              onChange={(e) => setNewStore({...newStore, region: e.target.value})}
              className="w-full p-2 border rounded"
            >
              <option value="">選択してください</option>
              <option value="北海道">北海道</option>
              <option value="東北">東北</option>
              <option value="関東">関東</option>
              <option value="中部">中部</option>
              <option value="関西">関西</option>
              <option value="中国">中国</option>
              <option value="四国">四国</option>
              <option value="九州">九州</option>
            </select>
          </div>
        </div>

        <button
          onClick={handleAddStore}
          disabled={!newStore.store_code || !newStore.store_name}
          className="bg-primary text-white px-6 py-2 rounded disabled:opacity-50"
        >
          追加
        </button>
      </div>

      {/* 店舗一覧 */}
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left">店舗コード</th>
              <th className="px-6 py-3 text-left">店舗名</th>
              <th className="px-6 py-3 text-left">リージョン</th>
              <th className="px-6 py-3 text-left">ステータス</th>
            </tr>
          </thead>
          <tbody>
            {stores.map(store => (
              <tr key={store.id} className="border-t">
                <td className="px-6 py-4">{store.store_code}</td>
                <td className="px-6 py-4">{store.store_name}</td>
                <td className="px-6 py-4">{store.region}</td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-sm ${
                    store.status === 'active'
                      ? 'bg-green-100 text-green-800'
                      : 'bg-gray-100 text-gray-800'
                  }`}>
                    {store.status === 'active' ? '有効' : '無効'}
                  </span>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  )
}
```

#### チェックリスト

- [ ] 店舗管理画面作成
- [ ] 店舗コード発行機能動作確認
- [ ] 店舗一覧表示確認

---

### Day 7: 認証状態管理 + ルーティング

#### タスク1: 認証状態管理

```tsx
// src/contexts/AuthContext.tsx
import { createContext, useContext, useEffect, useState } from 'react'
import { supabase } from '@/lib/supabase'
import type { User } from '@supabase/supabase-js'

interface AuthContextType {
  user: User | null
  profile: Profile | null
  loading: boolean
  signOut: () => Promise<void>
}

const AuthContext = createContext<AuthContextType>({} as AuthContextType)

export function AuthProvider({ children }: { children: React.ReactNode }) {
  const [user, setUser] = useState<User | null>(null)
  const [profile, setProfile] = useState<Profile | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // 初期セッション取得
    supabase.auth.getSession().then(({ data: { session } }) => {
      setUser(session?.user ?? null)
      if (session?.user) {
        fetchProfile(session.user.id)
      }
      setLoading(false)
    })

    // 認証状態変更を監視
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (_event, session) => {
        setUser(session?.user ?? null)
        if (session?.user) {
          fetchProfile(session.user.id)
        }
      }
    )

    return () => subscription.unsubscribe()
  }, [])

  const fetchProfile = async (userId: string) => {
    const { data } = await supabase
      .from('profiles')
      .select('*')
      .eq('id', userId)
      .single()
    setProfile(data)
  }

  const signOut = async () => {
    await supabase.auth.signOut()
    setUser(null)
    setProfile(null)
  }

  return (
    <AuthContext.Provider value={{ user, profile, loading, signOut }}>
      {children}
    </AuthContext.Provider>
  )
}

export const useAuth = () => useContext(AuthContext)
```

#### タスク2: ルーティング設定

```tsx
// src/App.tsx
import { BrowserRouter, Routes, Route, Navigate } from 'react-router-dom'
import { AuthProvider, useAuth } from '@/contexts/AuthContext'
import { LoginPage } from '@/components/Auth/LoginPage'
import { RegisterPage } from '@/components/Auth/RegisterPage'
import { HomePage } from '@/components/HomePage'
import { StoreManagement } from '@/components/Admin/StoreManagement'

function ProtectedRoute({ children }: { children: React.ReactNode }) {
  const { user, profile, loading } = useAuth()

  if (loading) return <div>Loading...</div>
  if (!user) return <Navigate to="/login" />
  if (!profile) return <Navigate to="/register" />

  return <>{children}</>
}

function AdminRoute({ children }: { children: React.ReactNode }) {
  const { profile } = useAuth()

  if (profile?.role !== 'admin') return <Navigate to="/" />

  return <>{children}</>
}

export default function App() {
  return (
    <BrowserRouter>
      <AuthProvider>
        <Routes>
          <Route path="/login" element={<LoginPage />} />
          <Route path="/register" element={<RegisterPage />} />

          <Route path="/" element={
            <ProtectedRoute>
              <HomePage />
            </ProtectedRoute>
          } />

          <Route path="/admin/stores" element={
            <ProtectedRoute>
              <AdminRoute>
                <StoreManagement />
              </AdminRoute>
            </ProtectedRoute>
          } />
        </Routes>
      </AuthProvider>
    </BrowserRouter>
  )
}
```

#### チェックリスト

- [ ] 認証状態管理実装
- [ ] ルーティング設定完了
- [ ] 未ログイン時のリダイレクト確認
- [ ] 管理者権限チェック確認

---

### Week 1まとめ & チェックポイント

#### 完成すべき機能

- [x] Supabaseプロジェクト作成
- [x] Google OAuth設定
- [x] データベース設計・構築
- [x] RLS設定
- [x] Google認証UI
- [x] 店舗コード入力フロー
- [x] 店舗管理画面

#### 動作確認

```
テストシナリオ:
1. Googleアカウントでログイン
2. 店舗コード「STORE_001」を入力
3. 名前を入力して登録完了
4. ホーム画面が表示される
5. 管理者アカウントで店舗管理画面にアクセス
6. 新規店舗「STORE_004」を追加
```

#### 成果物

✅ 認証付きアプリ（店舗コード必須）

**次週**: Week 2 - 6シナリオ構築 + 音声データ処理

---

## 📅 Week 2: 6シナリオ構築 + 音声データ処理

**目標**: 6シナリオ + RAGデータ600-800件

### Day 1-2: 6シナリオのJSON作成

#### タスク1: シナリオファイル準備

```bash
# ディレクトリ作成
mkdir -p scenarios
cd scenarios
```

#### タスク2: meeting_1st.json（1次面談）

```json
{
  "id": "meeting_1st",
  "title": "1次面談",
  "phase": "初回接触",
  "persona": {
    "customer_role": "SNS担当者 or 経営者",
    "tone": "警戒心あり、慎重",
    "relationship": "初対面",
    "knowledge_level": "動画制作について不明",
    "decision_power": "情報収集段階",
    "pain_points": [
      "SNSの反応が悪い",
      "動画制作の知識がない",
      "予算が不明確",
      "他社との比較をしたい"
    ]
  },
  "guidelines": [
    "最初は距離を置いた対応",
    "専門用語には「それって何ですか？」と質問",
    "予算の話にはまだ具体的に答えない",
    "「検討します」「他社も見ています」と保留",
    "信頼関係がないため、深い話はしない"
  ],
  "expected_flow": [
    "挨拶・自己紹介",
    "相談内容の確認",
    "現状のSNS運用ヒアリング",
    "課題の表面的な共有",
    "次回面談の約束"
  ],
  "utterances": [
    {
      "speaker": "営業",
      "text": "本日はお時間ありがとうございます。SNS動画制作にご興味をお持ちいただいたとのことで。"
    },
    {
      "speaker": "お客様",
      "text": "はい。SNS動画って、どれくらいの予算でできるんでしょうか？"
    },
    {
      "speaker": "営業",
      "text": "ご予算はどれくらいでお考えですか？"
    },
    {
      "speaker": "お客様",
      "text": "まだ分からなくて...検討段階なんです。"
    }
  ]
}
```

#### タスク3: 残り5シナリオも同様に作成

- [ ] meeting_1_5.json（1.5次面談）
- [ ] meeting_2nd.json（2次面談）
- [ ] meeting_3rd.json（3次面談）
- [ ] kickoff.json（キックオフMTG）
- [ ] upsell.json（追加営業）

#### タスク4: index.json（シナリオ一覧）

```json
{
  "default_id": "meeting_1st",
  "scenarios": [
    {
      "id": "meeting_1st",
      "file": "meeting_1st.json",
      "title": "1次面談",
      "description": "初回接触・関係構築",
      "enabled": true,
      "tags": ["初回", "ヒアリング", "信頼構築"]
    },
    {
      "id": "meeting_1_5",
      "file": "meeting_1_5.json",
      "title": "1.5次面談",
      "description": "情報交換・信頼構築",
      "enabled": true,
      "tags": ["関係構築", "詳細確認"]
    },
    {
      "id": "meeting_2nd",
      "file": "meeting_2nd.json",
      "title": "2次面談",
      "description": "提案・検討",
      "enabled": true,
      "tags": ["提案", "事例紹介"]
    },
    {
      "id": "meeting_3rd",
      "file": "meeting_3rd.json",
      "title": "3次面談",
      "description": "クロージング",
      "enabled": true,
      "tags": ["クロージング", "契約"]
    },
    {
      "id": "kickoff",
      "file": "kickoff.json",
      "title": "キックオフMTG",
      "description": "プロジェクト開始",
      "enabled": true,
      "tags": ["キックオフ", "協力関係"]
    },
    {
      "id": "upsell",
      "file": "upsell.json",
      "title": "追加営業",
      "description": "既存顧客への提案",
      "enabled": true,
      "tags": ["追加営業", "アップセル"]
    }
  ]
}
```

#### チェックリスト

- [ ] 6シナリオファイル作成完了
- [ ] index.json作成完了
- [ ] JSONフォーマット検証（https://jsonlint.com）

---

### Day 3-4: 音声データ処理（30本）

#### 事前準備

```
音声データの配置:
/audio_data/
├─ meeting_1st/
│  ├─ 1st_001.mp3 (30分)
│  ├─ 1st_002.mp3 (30分)
│  ├─ 1st_003.mp3 (30分)
│  ├─ 1st_004.mp3 (30分)
│  └─ 1st_005.mp3 (30分)
├─ meeting_1_5/
│  └─ ...（5本）
├─ meeting_2nd/
│  └─ ...（5本）
├─ meeting_3rd/
│  └─ ...（5本）
├─ kickoff/
│  └─ ...（5本）
└─ upsell/
   └─ ...（5本）
```

#### タスク1: Whisper文字起こしスクリプト

```python
# tools/transcribe_audio.py
import os
from openai import OpenAI
from pathlib import Path
import json

client = OpenAI()

def transcribe_audio_file(audio_path: str) -> str:
    """音声ファイルをWhisper APIで文字起こし"""
    with open(audio_path, 'rb') as audio_file:
        transcript = client.audio.transcriptions.create(
            model="whisper-1",
            file=audio_file,
            language="ja"
        )
    return transcript.text

def process_scenario_folder(scenario_folder: str, output_folder: str):
    """シナリオフォルダ内の全音声ファイルを処理"""
    audio_files = sorted(Path(scenario_folder).glob('*.mp3'))

    for audio_file in audio_files:
        print(f"Processing: {audio_file.name}")

        # 文字起こし
        transcript = transcribe_audio_file(str(audio_file))

        # 結果を保存
        output_path = Path(output_folder) / f"{audio_file.stem}.txt"
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(transcript)

        print(f"Saved: {output_path}")

# 実行
scenarios = [
    'meeting_1st',
    'meeting_1_5',
    'meeting_2nd',
    'meeting_3rd',
    'kickoff',
    'upsell'
]

for scenario in scenarios:
    print(f"\n=== {scenario} ===")
    process_scenario_folder(
        f'audio_data/{scenario}',
        f'transcripts/{scenario}'
    )
```

#### タスク2: 話者分離スクリプト

```python
# tools/speaker_diarization.py
import re
from openai import OpenAI

client = OpenAI()

def separate_speakers_with_gpt(transcript: str, scenario_id: str) -> list:
    """GPT-4で話者分離"""

    prompt = f"""
以下はSNS動画制作の無料相談の文字起こしです。
シナリオ: {scenario_id}

文字起こし:
{transcript}

このテキストを営業（相談員）と顧客（相談者）の発言に分離してください。
各発言を以下のJSON形式で出力してください:

[
  {{"speaker": "営業", "text": "発言内容"}},
  {{"speaker": "顧客", "text": "発言内容"}}
]
"""

    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "あなたは会話の話者分離を行う専門家です。"},
            {"role": "user", "content": prompt}
        ],
        temperature=0
    )

    # JSON抽出
    content = response.choices[0].message.content
    json_match = re.search(r'\[.*\]', content, re.DOTALL)
    if json_match:
        return json.loads(json_match.group())
    return []

# 実行例
transcript_path = 'transcripts/meeting_1st/1st_001.txt'
with open(transcript_path, 'r', encoding='utf-8') as f:
    transcript = f.read()

separated = separate_speakers_with_gpt(transcript, 'meeting_1st')

# 保存
output_path = 'separated/meeting_1st/1st_001.json'
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(separated, f, ensure_ascii=False, indent=2)
```

#### タスク3: タグ付けスクリプト

```python
# tools/tag_conversations.py
def tag_utterances(utterances: list, scenario_id: str) -> list:
    """発言にタグを付与"""

    for i, utterance in enumerate(utterances):
        if utterance['speaker'] == '顧客':
            # タグ付け
            text = utterance['text'].lower()
            tags = []

            # シーン判定
            if any(word in text for word in ['こんにちは', 'よろしく', '初めまして']):
                tags.append('挨拶')
            if any(word in text for word in ['予算', '費用', '価格', 'お金']):
                tags.append('予算')
            if any(word in text for word in ['期間', 'いつ', 'スケジュール']):
                tags.append('期間')
            if any(word in text for word in ['事例', '実績', '他の会社']):
                tags.append('事例')
            if any(word in text for word in ['検討', '考え', '相談']):
                tags.append('検討')

            # 感情判定
            emotion = 'neutral'
            if any(word in text for word in ['不安', '心配', '大丈夫']):
                emotion = '不安'
            if any(word in text for word in ['興味', '良い', '素晴らしい']):
                emotion = '興味'
            if any(word in text for word in ['厳しい', '難しい', '無理']):
                emotion = '懸念'

            # 関係性レベル（シナリオに基づく）
            relationship_level = {
                'meeting_1st': 1,
                'meeting_1_5': 2,
                'meeting_2nd': 3,
                'meeting_3rd': 4,
                'kickoff': 5,
                'upsell': 5
            }.get(scenario_id, 1)

            # メタデータ追加
            utterance['scenario_id'] = scenario_id
            utterance['tags'] = tags
            utterance['emotion'] = emotion
            utterance['relationship_level'] = relationship_level
            utterance['index'] = i

    return [u for u in utterances if u['speaker'] == '顧客']
```

#### チェックリスト

- [ ] 30本全ての音声を文字起こし
- [ ] 話者分離完了
- [ ] タグ付け完了
- [ ] JSON形式で保存

---

### Day 5: RAGインデックス構築

#### タスク1: 顧客発言の抽出・統合

```python
# tools/build_rag_index.py
import json
from pathlib import Path
from openai import OpenAI
import numpy as np
import faiss

client = OpenAI()

def load_all_customer_utterances() -> list:
    """全シナリオの顧客発言を読み込み"""
    all_utterances = []

    scenarios = ['meeting_1st', 'meeting_1_5', 'meeting_2nd',
                 'meeting_3rd', 'kickoff', 'upsell']

    for scenario in scenarios:
        folder = Path(f'separated/{scenario}')
        for json_file in folder.glob('*.json'):
            with open(json_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                # 顧客発言のみ抽出
                customer_utterances = [
                    u for u in data if u['speaker'] == '顧客'
                ]
                all_utterances.extend(customer_utterances)

    return all_utterances

def generate_embeddings(texts: list) -> np.ndarray:
    """テキストのEmbeddingを生成"""
    response = client.embeddings.create(
        model="text-embedding-3-large",
        input=texts
    )

    embeddings = np.array([
        item.embedding for item in response.data
    ], dtype=np.float32)

    return embeddings

def build_faiss_index(utterances: list):
    """FAISSインデックスを構築"""

    # テキスト抽出
    texts = [u['text'] for u in utterances]

    # Embedding生成
    print(f"Generating embeddings for {len(texts)} utterances...")
    embeddings = generate_embeddings(texts)

    # FAISSインデックス作成
    dimension = embeddings.shape[1]  # 3072 for text-embedding-3-large
    index = faiss.IndexFlatL2(dimension)
    index.add(embeddings)

    # 保存
    faiss.write_index(index, 'rag_index/rag_index.faiss')

    # メタデータ保存
    with open('rag_index/rag_metadata.json', 'w', encoding='utf-8') as f:
        json.dump(utterances, f, ensure_ascii=False, indent=2)

    print(f"FAISS index created with {len(texts)} entries")
    return index

# 実行
utterances = load_all_customer_utterances()
print(f"Total customer utterances: {len(utterances)}")
build_faiss_index(utterances)
```

#### タスク2: RAG検索関数（バックエンド統合）

```python
# app.py に追加
import faiss
import numpy as np

# グローバル変数
RAG_INDEX = None
RAG_METADATA = None

def load_rag_index():
    """RAGインデックスを読み込み"""
    global RAG_INDEX, RAG_METADATA

    try:
        RAG_INDEX = faiss.read_index('rag_index/rag_index.faiss')
        with open('rag_index/rag_metadata.json', 'r', encoding='utf-8') as f:
            RAG_METADATA = json.load(f)
        print(f"RAG index loaded: {len(RAG_METADATA)} entries")
    except Exception as e:
        print(f"RAG index not found: {e}")

def search_rag_by_scenario(query: str, scenario_id: str, top_k: int = 3):
    """シナリオ別RAG検索"""

    # シナリオでフィルタリング
    filtered_metadata = [
        (i, m) for i, m in enumerate(RAG_METADATA)
        if m.get('scenario_id') == scenario_id
    ]

    if not filtered_metadata:
        return []

    # クエリのEmbedding生成
    response = client.embeddings.create(
        model="text-embedding-3-large",
        input=[query]
    )
    query_embedding = np.array([response.data[0].embedding], dtype=np.float32)

    # 全体から検索
    distances, indices = RAG_INDEX.search(query_embedding, top_k * 3)

    # シナリオに合致するものだけ抽出
    results = []
    for dist, idx in zip(distances[0], indices[0]):
        if RAG_METADATA[idx].get('scenario_id') == scenario_id:
            results.append({
                'text': RAG_METADATA[idx]['text'],
                'tags': RAG_METADATA[idx].get('tags', []),
                'emotion': RAG_METADATA[idx].get('emotion', ''),
                'distance': float(dist)
            })
            if len(results) >= top_k:
                break

    return results

# アプリ起動時に読み込み
load_rag_index()
```

#### チェックリスト

- [ ] 顧客発言600-800件抽出
- [ ] Embedding生成完了
- [ ] FAISSインデックス構築完了
- [ ] シナリオ別検索動作確認

---

### Day 6-7: シナリオ別検索機能の実装

#### タスク1: /api/chat エンドポイント改修

```python
# app.py
@app.route('/api/chat', methods=['POST'])
def chat():
    data = request.json
    user_message = data.get('message')
    history = data.get('history', [])
    scenario_id = data.get('scenario_id', 'meeting_1st')

    # シナリオ詳細を読み込み
    scenario = load_scenario_object(scenario_id)

    # RAG検索（シナリオ別）
    rag_results = search_rag_by_scenario(user_message, scenario_id, top_k=3)

    # プロンプト構築
    system_prompt = f"""あなたはSNS動画制作の無料相談に来た相談者役です。

【シナリオ】
{scenario['title']} - {scenario['phase']}

【顧客設定】
{json.dumps(scenario['persona'], ensure_ascii=False, indent=2)}

【返答ガイドライン】
{chr(10).join('- ' + g for g in scenario['guidelines'])}

【過去の実例パターン（類似した相談者の発言）】
"""

    if rag_results:
        for i, result in enumerate(rag_results, 1):
            system_prompt += f"\n{i}. {result['text']}"

    # GPT-4o-mini呼び出し
    messages = [{"role": "system", "content": system_prompt}]

    # Few-shot（シナリオのutterances）
    for utterance in scenario.get('utterances', [])[:4]:
        role = "assistant" if utterance['speaker'] == 'お客様' else "user"
        messages.append({"role": role, "content": utterance['text']})

    # 会話履歴
    for msg in history:
        role = "assistant" if msg['speaker'] == 'お客様' else "user"
        messages.append({"role": role, "content": msg['text']})

    # 現在のユーザーメッセージ
    messages.append({"role": "user", "content": user_message})

    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=messages,
        temperature=0.7,
        max_tokens=200
    )

    ai_response = response.choices[0].message.content

    return jsonify({
        "success": True,
        "response": ai_response,
        "scenario_id": scenario_id,
        "rag_used": len(rag_results) > 0
    })
```

#### チェックリスト

- [ ] /api/chatエンドポイント改修完了
- [ ] シナリオ別RAG検索動作確認
- [ ] 6シナリオ全てで動作確認

---

### Week 2まとめ & チェックポイント

#### 完成すべき機能

- [x] 6シナリオのJSON作成
- [x] 音声データ30本の処理
- [x] RAGインデックス構築（600-800件）
- [x] シナリオ別検索機能

#### 動作確認

```
テストシナリオ:
1. シナリオ「1次面談」を選択
2. 「SNS動画の予算はどれくらいですか？」と質問
3. AI相談者が「1次面談」の雰囲気で応答
   （警戒心あり、慎重な返答）
4. シナリオ「追加営業」に切り替え
5. 同じ質問をする
6. AI相談者が「追加営業」の雰囲気で応答
   （フランク、親しみやすい返答）
```

#### 成果物

✅ 6シナリオ + RAGデータ600-800件

**次週**: Week 3 - データ永続化

---

## 📅 Week 3: データ永続化 + シナリオ切替UI

**目標**: データが残るアプリ（6シナリオ対応）

### Day 1-2: バックエンド - Supabase統合

#### タスク1: Supabase Pythonクライアント

```bash
# 依存関係追加
pip install supabase
```

```python
# app.py
from supabase import create_client, Client
import os

# Supabaseクライアント初期化
SUPABASE_URL = os.getenv('SUPABASE_URL')
SUPABASE_KEY = os.getenv('SUPABASE_SERVICE_ROLE_KEY')
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
```

#### タスク2: 会話履歴保存API

```python
# app.py
@app.route('/api/conversations/save', methods=['POST'])
def save_conversation():
    """会話履歴を保存"""
    data = request.json

    # リクエストボディ
    user_id = data.get('user_id')
    store_id = data.get('store_id')
    scenario_id = data.get('scenario_id')
    scenario_title = data.get('scenario_title')
    messages = data.get('messages')  # [{speaker, text, timestamp}]
    duration_seconds = data.get('duration_seconds')

    # Supabaseに保存
    result = supabase.table('conversations').insert({
        'user_id': user_id,
        'store_id': store_id,
        'scenario_id': scenario_id,
        'scenario_title': scenario_title,
        'messages': messages,
        'duration_seconds': duration_seconds
    }).execute()

    return jsonify({
        "success": True,
        "conversation_id": result.data[0]['id']
    })

@app.route('/api/conversations', methods=['GET'])
def get_conversations():
    """会話履歴を取得"""
    user_id = request.args.get('user_id')
    scenario_id = request.args.get('scenario_id')  # オプション

    query = supabase.table('conversations').select('*').eq('user_id', user_id)

    if scenario_id:
        query = query.eq('scenario_id', scenario_id)

    result = query.order('created_at', desc=True).execute()

    return jsonify({
        "success": True,
        "conversations": result.data
    })
```

#### タスク3: 評価履歴保存API

```python
# app.py
@app.route('/api/evaluations/save', methods=['POST'])
def save_evaluation():
    """評価履歴を保存"""
    data = request.json

    conversation_id = data.get('conversation_id')
    user_id = data.get('user_id')
    store_id = data.get('store_id')
    scenario_id = data.get('scenario_id')
    scores = data.get('scores')
    comments = data.get('comments')

    # 総合スコア計算
    total_score = sum(scores.values())
    average_score = total_score / len(scores)

    result = supabase.table('evaluations').insert({
        'conversation_id': conversation_id,
        'user_id': user_id,
        'store_id': store_id,
        'scenario_id': scenario_id,
        'scores': scores,
        'total_score': total_score,
        'average_score': average_score,
        'comments': comments
    }).execute()

    return jsonify({
        "success": True,
        "evaluation_id": result.data[0]['id']
    })

@app.route('/api/evaluations', methods=['GET'])
def get_evaluations():
    """評価履歴を取得"""
    user_id = request.args.get('user_id')
    scenario_id = request.args.get('scenario_id')

    query = supabase.table('evaluations').select('*').eq('user_id', user_id)

    if scenario_id:
        query = query.eq('scenario_id', scenario_id)

    result = query.order('created_at', desc=True).execute()

    return jsonify({
        "success": True,
        "evaluations": result.data
    })
```

#### チェックリスト

- [ ] Supabase Pythonクライアント統合
- [ ] 会話保存API動作確認
- [ ] 評価保存API動作確認

---

### Day 3-4: フロントエンド - 自動保存機能

#### タスク1: 会話終了時の自動保存

```tsx
// src/App.tsx
import { useAuth } from '@/contexts/AuthContext'
import { supabase } from '@/lib/supabase'

export function App() {
  const { user, profile } = useAuth()
  const [messages, setMessages] = useState([])
  const [currentScenarioId, setCurrentScenarioId] = useState('meeting_1st')

  // 会話保存関数
  const saveConversation = async () => {
    if (!messages.length) return

    const response = await fetch('/api/conversations/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: user.id,
        store_id: profile.store_id,
        scenario_id: currentScenarioId,
        scenario_title: scenarios.find(s => s.id === currentScenarioId)?.title,
        messages: messages.map(m => ({
          speaker: m.role === 'user' ? '営業' : '顧客',
          text: m.text,
          timestamp: m.timestamp
        })),
        duration_seconds: calculateDuration(messages)
      })
    })

    const data = await response.json()
    return data.conversation_id
  }

  // 講評表示時に自動保存
  const handleShowEvaluation = async () => {
    // 会話を保存
    const conversationId = await saveConversation()

    // 講評生成
    const evaluation = await getEvaluation(messages)

    // 講評を保存
    await fetch('/api/evaluations/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        conversation_id: conversationId,
        user_id: user.id,
        store_id: profile.store_id,
        scenario_id: currentScenarioId,
        scores: evaluation.scores,
        comments: evaluation.comments
      })
    })

    setShowEvaluation(true)
    setEvaluation(evaluation)
  }

  return (
    // ... UI
  )
}
```

#### チェックリスト

- [ ] 会話自動保存実装
- [ ] 評価自動保存実装
- [ ] 保存成功トースト表示

---

### Day 5: シナリオ選択UI

#### タスク1: ヘッダーにシナリオ選択

```tsx
// src/components/Header.tsx
import { Select } from '@/components/ui/Select'

export function Header({
  currentScenarioId,
  onScenarioChange
}: {
  currentScenarioId: string
  onScenarioChange: (id: string) => void
}) {
  const { profile, signOut } = useAuth()
  const [scenarios, setScenarios] = useState([])

  useEffect(() => {
    fetch('/api/scenarios')
      .then(res => res.json())
      .then(data => setScenarios(data.scenarios))
  }, [])

  return (
    <header className="flex items-center justify-between p-4 border-b">
      <div className="flex items-center gap-4">
        <img src={profile.avatar_url} className="w-10 h-10 rounded-full" />
        <div>
          <p className="font-semibold">{profile.display_name}</p>
          <p className="text-sm text-gray-600">{profile.store_name}</p>
        </div>
      </div>

      <div className="flex items-center gap-4">
        <Select
          value={currentScenarioId}
          onChange={(e) => onScenarioChange(e.target.value)}
          className="w-64"
        >
          {scenarios.map(scenario => (
            <option key={scenario.id} value={scenario.id}>
              {scenario.title} - {scenario.description}
            </option>
          ))}
        </Select>

        <button onClick={signOut} className="text-sm text-gray-600">
          ログアウト
        </button>
      </div>
    </header>
  )
}
```

#### タスク2: シナリオ切り替え時の処理

```tsx
// src/App.tsx
const handleScenarioChange = async (newScenarioId: string) => {
  // 現在の会話を保存（メッセージがあれば）
  if (messages.length > 0) {
    const shouldSave = confirm('現在の会話を保存しますか？')
    if (shouldSave) {
      await saveConversation()
    }
  }

  // シナリオ変更
  setCurrentScenarioId(newScenarioId)
  setMessages([])

  // 新しいシナリオの最初のメッセージを取得
  const scenarioData = await fetch(`/api/scenarios/${newScenarioId}`)
    .then(res => res.json())

  const firstCustomerMsg = scenarioData.scenario.utterances
    .find(u => u.speaker === 'お客様')

  if (firstCustomerMsg) {
    setMessages([{
      role: 'bot',
      text: firstCustomerMsg.text,
      timestamp: new Date().toISOString()
    }])
    speakText(firstCustomerMsg.text)
  }
}
```

#### チェックリスト

- [ ] シナリオ選択UI実装
- [ ] シナリオ切り替え動作確認
- [ ] 会話保存確認ダイアログ実装

---

### Week 3まとめ & チェックポイント

#### 完成すべき機能

- [x] Supabase統合（バックエンド）
- [x] 会話自動保存
- [x] 評価自動保存
- [x] シナリオ選択UI
- [x] シナリオ切り替え機能

#### 動作確認

```
テストシナリオ:
1. 「1次面談」でロープレ実施
2. 講評を表示
3. データベースに保存されることを確認
4. 「2次面談」に切り替え
5. 新しい会話が開始されることを確認
```

#### 成果物

✅ データが残るアプリ（6シナリオ対応）

**次週**: Week 4 - 履歴閲覧 + CSV出力

---

## 📅 Week 4: 履歴閲覧 + CSV出力

**目標**: 成長可視化機能（履歴・グラフ・CSV）

### Day 1-2: 履歴閲覧UI

#### タスク1: 履歴一覧ページ

```tsx
// src/components/History/HistoryPage.tsx
import { useState, useEffect } from 'react'
import { useAuth } from '@/contexts/AuthContext'
import { format } from 'date-fns'
import { ja } from 'date-fns/locale'

export function HistoryPage() {
  const { user, profile } = useAuth()
  const [conversations, setConversations] = useState([])
  const [selectedScenario, setSelectedScenario] = useState('all')
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetchHistory()
  }, [selectedScenario])

  const fetchHistory = async () => {
    setLoading(true)
    const params = new URLSearchParams({ user_id: user.id })
    if (selectedScenario !== 'all') {
      params.append('scenario_id', selectedScenario)
    }

    const response = await fetch(`/api/conversations?${params}`)
    const data = await response.json()

    // 評価データも取得
    const evaluationsResponse = await fetch(`/api/evaluations?${params}`)
    const evaluationsData = await evaluationsResponse.json()

    // 会話と評価をマージ
    const merged = data.conversations.map(conv => {
      const evaluation = evaluationsData.evaluations.find(
        e => e.conversation_id === conv.id
      )
      return { ...conv, evaluation }
    })

    setConversations(merged)
    setLoading(false)
  }

  return (
    <div className="max-w-6xl mx-auto p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold">練習履歴</h2>

        {/* シナリオフィルター */}
        <select
          value={selectedScenario}
          onChange={(e) => setSelectedScenario(e.target.value)}
          className="p-2 border rounded"
        >
          <option value="all">全シナリオ</option>
          <option value="meeting_1st">1次面談</option>
          <option value="meeting_1_5">1.5次面談</option>
          <option value="meeting_2nd">2次面談</option>
          <option value="meeting_3rd">3次面談</option>
          <option value="kickoff">キックオフMTG</option>
          <option value="upsell">追加営業</option>
        </select>
      </div>

      {loading ? (
        <div className="text-center py-12">読み込み中...</div>
      ) : (
        <div className="space-y-4">
          {conversations.map(conv => (
            <HistoryCard key={conv.id} conversation={conv} />
          ))}
        </div>
      )}
    </div>
  )
}

function HistoryCard({ conversation }) {
  const [expanded, setExpanded] = useState(false)

  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <span className="px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">
              {conversation.scenario_title}
            </span>
            <span className="text-sm text-gray-500">
              {format(new Date(conversation.created_at), 'yyyy/MM/dd HH:mm', { locale: ja })}
            </span>
            <span className="text-sm text-gray-500">
              {Math.floor(conversation.duration_seconds / 60)}分{conversation.duration_seconds % 60}秒
            </span>
          </div>

          {conversation.evaluation && (
            <div className="flex items-center gap-4 mt-3">
              <div className="text-2xl font-bold text-primary">
                {conversation.evaluation.average_score.toFixed(1)}点
              </div>
              <div className="flex gap-2">
                {Object.entries(conversation.evaluation.scores).map(([key, value]) => (
                  <div key={key} className="text-sm">
                    <span className="text-gray-600">{getScoreLabel(key)}:</span>
                    <span className="font-semibold ml-1">{value}点</span>
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>

        <button
          onClick={() => setExpanded(!expanded)}
          className="text-sm text-primary hover:underline"
        >
          {expanded ? '閉じる' : '詳細を見る'}
        </button>
      </div>

      {expanded && (
        <div className="mt-6 border-t pt-4">
          <h4 className="font-semibold mb-3">会話内容</h4>
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {conversation.messages.map((msg, i) => (
              <div
                key={i}
                className={`p-3 rounded-lg ${
                  msg.speaker === '営業'
                    ? 'bg-blue-50 text-blue-900'
                    : 'bg-gray-50 text-gray-900'
                }`}
              >
                <div className="text-xs text-gray-500 mb-1">{msg.speaker}</div>
                <div>{msg.text}</div>
              </div>
            ))}
          </div>

          {conversation.evaluation && (
            <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
              <h4 className="font-semibold mb-2">講評</h4>
              <div className="space-y-2 text-sm">
                {Object.entries(conversation.evaluation.comments).map(([key, comment]) => (
                  <div key={key}>
                    <span className="font-semibold">{getScoreLabel(key)}:</span>
                    <p className="text-gray-700 mt-1">{comment}</p>
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>
      )}
    </div>
  )
}

function getScoreLabel(key: string): string {
  const labels = {
    hearing: 'ヒアリング力',
    empathy: '共感力',
    proposal: '提案力',
    trust: '信頼構築力'
  }
  return labels[key] || key
}
```

#### チェックリスト

- [ ] 履歴一覧UI実装
- [ ] シナリオフィルター動作確認
- [ ] 詳細表示動作確認

---

### Day 3: 成長グラフ表示

#### タスク1: Chart.js統合

```bash
npm install chart.js react-chartjs-2
```

```tsx
// src/components/History/GrowthChart.tsx
import { Line } from 'react-chartjs-2'
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
  PointElement,
  LineElement,
  Title,
  Tooltip,
  Legend
} from 'chart.js'

ChartJS.register(
  CategoryScale,
  LinearScale,
  PointElement,
  LineElement,
  Title,
  Tooltip,
  Legend
)

export function GrowthChart({ evaluations }) {
  const data = {
    labels: evaluations.map((e, i) => `${i + 1}回目`),
    datasets: [
      {
        label: 'ヒアリング力',
        data: evaluations.map(e => e.scores.hearing),
        borderColor: 'rgb(255, 99, 132)',
        backgroundColor: 'rgba(255, 99, 132, 0.5)',
      },
      {
        label: '共感力',
        data: evaluations.map(e => e.scores.empathy),
        borderColor: 'rgb(54, 162, 235)',
        backgroundColor: 'rgba(54, 162, 235, 0.5)',
      },
      {
        label: '提案力',
        data: evaluations.map(e => e.scores.proposal),
        borderColor: 'rgb(75, 192, 192)',
        backgroundColor: 'rgba(75, 192, 192, 0.5)',
      },
      {
        label: '信頼構築力',
        data: evaluations.map(e => e.scores.trust),
        borderColor: 'rgb(153, 102, 255)',
        backgroundColor: 'rgba(153, 102, 255, 0.5)',
      },
    ],
  }

  const options = {
    responsive: true,
    plugins: {
      legend: {
        position: 'top' as const,
      },
      title: {
        display: true,
        text: 'スキル成長グラフ',
      },
    },
    scales: {
      y: {
        min: 0,
        max: 100,
      },
    },
  }

  return <Line options={options} data={data} />
}
```

#### チェックリスト

- [ ] Chart.js統合完了
- [ ] 成長グラフ表示確認
- [ ] レスポンシブ対応確認

---

### Day 4-5: CSV出力機能

#### タスク1: CSV出力API

```python
# app.py
import csv
from io import StringIO
from flask import Response

@app.route('/api/export/csv', methods=['GET'])
def export_csv():
    """評価履歴をCSV出力"""
    user_id = request.args.get('user_id')
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')

    # データ取得
    query = supabase.table('evaluations')\
        .select('*, conversations(scenario_title, created_at, messages)')\
        .eq('user_id', user_id)

    if start_date:
        query = query.gte('created_at', start_date)
    if end_date:
        query = query.lte('created_at', end_date)

    result = query.order('created_at').execute()

    # CSV生成
    output = StringIO()
    writer = csv.writer(output)

    # ヘッダー
    writer.writerow([
        '日時',
        'シナリオ',
        'ヒアリング力',
        '共感力',
        '提案力',
        '信頼構築力',
        '平均点',
        '会話ターン数'
    ])

    # データ行
    for item in result.data:
        writer.writerow([
            item['created_at'],
            item['conversations']['scenario_title'],
            item['scores']['hearing'],
            item['scores']['empathy'],
            item['scores']['proposal'],
            item['scores']['trust'],
            item['average_score'],
            len(item['conversations']['messages'])
        ])

    output.seek(0)

    return Response(
        output.getvalue(),
        mimetype='text/csv',
        headers={
            'Content-Disposition': 'attachment; filename=roleplay_history.csv'
        }
    )
```

#### タスク2: フロントエンドCSV出力ボタン

```tsx
// src/components/History/HistoryPage.tsx
const handleExportCSV = async () => {
  const params = new URLSearchParams({ user_id: user.id })

  const response = await fetch(`/api/export/csv?${params}`)
  const blob = await response.blob()

  const url = window.URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `roleplay_history_${format(new Date(), 'yyyyMMdd')}.csv`
  a.click()
  window.URL.revokeObjectURL(url)
}

// UIに追加
<button
  onClick={handleExportCSV}
  className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600"
>
  CSV出力
</button>
```

#### チェックリスト

- [ ] CSV出力API実装
- [ ] フロントエンドボタン実装
- [ ] CSV形式確認（Excel互換）

---

### Week 4まとめ

✅ 成長可視化機能（履歴・グラフ・CSV）

**次週**: Week 5 - 評価精度向上

---

## 📅 Week 5: 評価精度向上

**目標**: 講師評価80%一致

### Day 1-2: Few-shot評価プロンプト改善

#### タスク1: 評価用プロンプトテンプレート作成

```python
# evaluation_prompts.py
EVALUATION_PROMPT_TEMPLATE = """
あなたはSNS動画制作会社の営業トレーナーです。
以下の営業ロープレを評価してください。

【シナリオ】
{scenario_title} - {scenario_phase}

【評価基準】
1. ヒアリング力 (0-100点)
   - オープンクエスチョンの使用
   - 相手の本音を引き出せているか
   - 適切な相槌・リアクション

2. 共感力 (0-100点)
   - 相手の立場に立った発言
   - 感情に寄り添っているか
   - 信頼関係構築への配慮

3. 提案力 (0-100点)
   - 課題に対する適切な提案
   - 具体性・実現可能性
   - タイミングの適切さ

4. 信頼構築力 (0-100点)
   - 誠実な対応
   - 専門性の提示
   - 長期的関係性への配慮

【良い例（90点以上）】
営業: 「現在のSNS運用で、一番困っていることは何ですか?」
顧客: 「投稿しても反応が少なくて...」
営業: 「それは辛いですよね。どれくらい投稿されていますか?」
→ オープンクエスチョン、共感、深掘りが適切

【悪い例（50点以下）】
営業: 「弊社のサービスは月10万円からです。いかがですか?」
顧客: 「高いですね...」
営業: 「では8万円でどうですか?」
→ ヒアリング不足、一方的な提案、値引き先行

【会話履歴】
{conversation_transcript}

【評価出力形式】
{{
  "scores": {{
    "hearing": 85,
    "empathy": 78,
    "proposal": 82,
    "trust": 90
  }},
  "comments": {{
    "hearing": "具体的なコメント...",
    "empathy": "具体的なコメント...",
    "proposal": "具体的なコメント...",
    "trust": "具体的なコメント..."
  }},
  "overall_feedback": "全体的な講評..."
}}
"""

def generate_evaluation_prompt(conversation, scenario):
    """会話履歴から評価プロンプトを生成"""

    transcript = "\n".join([
        f"{msg['speaker']}: {msg['text']}"
        for msg in conversation
    ])

    return EVALUATION_PROMPT_TEMPLATE.format(
        scenario_title=scenario['title'],
        scenario_phase=scenario['phase'],
        conversation_transcript=transcript
    )
```

#### チェックリスト

- [ ] 評価プロンプトテンプレート作成
- [ ] Good/Bad例の追加
- [ ] Few-shot評価テスト

---

### Day 3: GPT-4評価エンジン統合

#### タスク1: 評価API改修

```python
# app.py
from evaluation_prompts import generate_evaluation_prompt
import json
import re

@app.route('/api/evaluate', methods=['POST'])
def evaluate():
    """GPT-4で評価を生成"""
    data = request.json
    messages = data.get('messages')
    scenario_id = data.get('scenario_id')

    # シナリオ読み込み
    scenario = load_scenario_object(scenario_id)

    # プロンプト生成
    prompt = generate_evaluation_prompt(messages, scenario)

    # GPT-4呼び出し
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "あなたは営業トレーニングの専門家です。"},
            {"role": "user", "content": prompt}
        ],
        temperature=0.3,
        max_tokens=1500
    )

    # JSON抽出
    content = response.choices[0].message.content
    json_match = re.search(r'\{.*\}', content, re.DOTALL)

    if json_match:
        evaluation = json.loads(json_match.group())

        # 平均点計算
        scores = evaluation['scores']
        average = sum(scores.values()) / len(scores)

        return jsonify({
            "success": True,
            "evaluation": {
                **evaluation,
                "average_score": round(average, 2),
                "total_score": sum(scores.values())
            }
        })

    return jsonify({"success": False, "error": "評価生成失敗"}), 500
```

#### チェックリスト

- [ ] GPT-4評価API実装
- [ ] JSON抽出ロジック確認
- [ ] エラーハンドリング実装

---

### Day 4-5: 評価精度テスト・チューニング

#### タスク1: テストデータ作成

```python
# tests/evaluation_test.py
import json

# 講師による手動評価データ（ゴールド標準）
GOLD_STANDARD_EVALUATIONS = [
    {
        "conversation_id": "test_001",
        "scenario_id": "meeting_1st",
        "human_scores": {
            "hearing": 85,
            "empathy": 78,
            "proposal": 70,
            "trust": 82
        },
        "messages": [
            # ... 会話データ
        ]
    },
    # ... 20-30件のテストデータ
]

def calculate_accuracy(ai_scores, human_scores):
    """AIと人間の評価の一致率を計算"""
    differences = []
    for key in ai_scores.keys():
        diff = abs(ai_scores[key] - human_scores[key])
        differences.append(diff)

    # 10点以内の差なら正解とする
    accuracy = sum(1 for d in differences if d <= 10) / len(differences)
    avg_difference = sum(differences) / len(differences)

    return {
        "accuracy": accuracy,
        "average_difference": avg_difference
    }

# テスト実行
for test in GOLD_STANDARD_EVALUATIONS:
    ai_evaluation = get_ai_evaluation(test['messages'], test['scenario_id'])
    result = calculate_accuracy(ai_evaluation['scores'], test['human_scores'])
    print(f"Test {test['conversation_id']}: Accuracy={result['accuracy']:.2%}")
```

#### タスク2: 評価基準の調整

```python
# プロンプトチューニング
# - Good/Bad例を10件ずつ追加
# - 評価軸の定義を具体化
# - シナリオ別の評価基準を追加

# 目標: 80%以上の一致率
```

#### チェックリスト

- [ ] テストデータ20件以上作成
- [ ] 精度テスト実施
- [ ] 80%以上の一致率達成

---

### Week 5まとめ

✅ 講師評価80%一致

**次週**: Week 6 - 店舗管理機能

---

## 📅 Week 6: 店舗管理機能

**目標**: フランチャイズ管理機能

### Day 1-2: 店舗別統計ダッシュボード

#### タスク1: 統計API

```python
# app.py
@app.route('/api/admin/stats/store/<store_id>', methods=['GET'])
def get_store_stats(store_id):
    """店舗別統計を取得"""

    # 所属ユーザー数
    users_count = supabase.table('profiles')\
        .select('id', count='exact')\
        .eq('store_id', store_id)\
        .execute().count

    # 総練習回数
    conversations_count = supabase.table('conversations')\
        .select('id', count='exact')\
        .eq('store_id', store_id)\
        .execute().count

    # 平均スコア
    evaluations = supabase.table('evaluations')\
        .select('average_score')\
        .eq('store_id', store_id)\
        .execute()

    avg_score = sum(e['average_score'] for e in evaluations.data) / len(evaluations.data) if evaluations.data else 0

    # シナリオ別練習回数
    scenario_stats = {}
    for scenario in ['meeting_1st', 'meeting_1_5', 'meeting_2nd', 'meeting_3rd', 'kickoff', 'upsell']:
        count = supabase.table('conversations')\
            .select('id', count='exact')\
            .eq('store_id', store_id)\
            .eq('scenario_id', scenario)\
            .execute().count
        scenario_stats[scenario] = count

    return jsonify({
        "success": True,
        "stats": {
            "users_count": users_count,
            "conversations_count": conversations_count,
            "average_score": round(avg_score, 2),
            "scenario_stats": scenario_stats
        }
    })
```

#### タスク2: 店舗ダッシュボードUI

```tsx
// src/components/Admin/StoreDashboard.tsx
export function StoreDashboard({ storeId }) {
  const [stats, setStats] = useState(null)

  useEffect(() => {
    fetch(`/api/admin/stats/store/${storeId}`)
      .then(res => res.json())
      .then(data => setStats(data.stats))
  }, [storeId])

  if (!stats) return <div>読み込み中...</div>

  return (
    <div className="grid grid-cols-4 gap-6">
      <StatCard
        title="所属ユーザー数"
        value={stats.users_count}
        icon="👥"
      />
      <StatCard
        title="総練習回数"
        value={stats.conversations_count}
        icon="💬"
      />
      <StatCard
        title="平均スコア"
        value={`${stats.average_score}点`}
        icon="⭐"
      />
      <StatCard
        title="今月の練習回数"
        value={stats.this_month_count}
        icon="📅"
      />

      <div className="col-span-4 bg-white p-6 rounded-lg shadow">
        <h3 className="text-lg font-semibold mb-4">シナリオ別練習回数</h3>
        <BarChart data={stats.scenario_stats} />
      </div>
    </div>
  )
}
```

#### チェックリスト

- [ ] 統計API実装
- [ ] ダッシュボードUI実装
- [ ] グラフ表示確認

---

### Day 3-4: 本部管理者機能

#### タスク1: 全店舗ランキング

```python
# app.py
@app.route('/api/admin/ranking', methods=['GET'])
def get_store_ranking():
    """全店舗のランキング"""

    # 全店舗の統計を取得
    stores = supabase.table('stores').select('*').execute()

    ranking = []
    for store in stores.data:
        evaluations = supabase.table('evaluations')\
            .select('average_score')\
            .eq('store_id', store['id'])\
            .execute()

        avg_score = sum(e['average_score'] for e in evaluations.data) / len(evaluations.data) if evaluations.data else 0

        ranking.append({
            "store_code": store['store_code'],
            "store_name": store['store_name'],
            "region": store['region'],
            "average_score": round(avg_score, 2),
            "practice_count": len(evaluations.data)
        })

    # スコア順にソート
    ranking.sort(key=lambda x: x['average_score'], reverse=True)

    return jsonify({
        "success": True,
        "ranking": ranking
    })
```

#### タスク2: ランキングUI

```tsx
// src/components/Admin/RankingPage.tsx
export function RankingPage() {
  const [ranking, setRanking] = useState([])

  useEffect(() => {
    fetch('/api/admin/ranking')
      .then(res => res.json())
      .then(data => setRanking(data.ranking))
  }, [])

  return (
    <div className="max-w-4xl mx-auto p-8">
      <h2 className="text-2xl font-bold mb-6">店舗別ランキング</h2>

      <table className="w-full bg-white rounded-lg shadow overflow-hidden">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-6 py-3 text-left">順位</th>
            <th className="px-6 py-3 text-left">店舗</th>
            <th className="px-6 py-3 text-left">リージョン</th>
            <th className="px-6 py-3 text-left">平均スコア</th>
            <th className="px-6 py-3 text-left">練習回数</th>
          </tr>
        </thead>
        <tbody>
          {ranking.map((store, index) => (
            <tr key={store.store_code} className="border-t">
              <td className="px-6 py-4">
                {index === 0 && '🥇'}
                {index === 1 && '🥈'}
                {index === 2 && '🥉'}
                {index > 2 && `${index + 1}位`}
              </td>
              <td className="px-6 py-4">
                <div>
                  <div className="font-semibold">{store.store_name}</div>
                  <div className="text-sm text-gray-500">{store.store_code}</div>
                </div>
              </td>
              <td className="px-6 py-4">{store.region}</td>
              <td className="px-6 py-4">
                <span className="text-xl font-bold text-primary">
                  {store.average_score}点
                </span>
              </td>
              <td className="px-6 py-4">{store.practice_count}回</td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}
```

#### チェックリスト

- [ ] ランキングAPI実装
- [ ] ランキングUI実装
- [ ] リアルタイム更新確認

---

### Day 5: 権限管理・ユーザー管理

#### タスク1: ユーザー一覧・編集

```tsx
// src/components/Admin/UserManagement.tsx
export function UserManagement() {
  const [users, setUsers] = useState([])

  const fetchUsers = async () => {
    const { data } = await supabase
      .from('profiles')
      .select('*, stores(store_name)')
      .order('created_at', { ascending: false })
    setUsers(data)
  }

  const handleRoleChange = async (userId, newRole) => {
    await supabase
      .from('profiles')
      .update({ role: newRole })
      .eq('id', userId)

    fetchUsers()
  }

  return (
    <div className="max-w-6xl mx-auto p-8">
      <h2 className="text-2xl font-bold mb-6">ユーザー管理</h2>

      <table className="w-full bg-white rounded-lg shadow">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-6 py-3 text-left">ユーザー</th>
            <th className="px-6 py-3 text-left">店舗</th>
            <th className="px-6 py-3 text-left">権限</th>
            <th className="px-6 py-3 text-left">最終ログイン</th>
            <th className="px-6 py-3 text-left">操作</th>
          </tr>
        </thead>
        <tbody>
          {users.map(user => (
            <tr key={user.id} className="border-t">
              <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                  <img src={user.avatar_url} className="w-10 h-10 rounded-full" />
                  <div>
                    <div className="font-semibold">{user.display_name}</div>
                    <div className="text-sm text-gray-500">{user.email}</div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4">{user.stores?.store_name}</td>
              <td className="px-6 py-4">
                <select
                  value={user.role}
                  onChange={(e) => handleRoleChange(user.id, e.target.value)}
                  className="p-2 border rounded"
                >
                  <option value="user">一般</option>
                  <option value="manager">店舗管理者</option>
                  <option value="admin">本部管理者</option>
                </select>
              </td>
              <td className="px-6 py-4">
                {user.last_active_at ? format(new Date(user.last_active_at), 'yyyy/MM/dd HH:mm') : '-'}
              </td>
              <td className="px-6 py-4">
                <button className="text-red-500 hover:underline">削除</button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}
```

#### チェックリスト

- [ ] ユーザー一覧UI実装
- [ ] 権限変更機能実装
- [ ] RLS動作確認

---

### Week 6まとめ

✅ フランチャイズ管理機能

**次週**: Week 7 - パフォーマンス最適化

---

## 📅 Week 7: パフォーマンス最適化

**目標**: 100店舗対応完了

### Day 1-2: データベース最適化

#### タスク1: インデックスチューニング

```sql
-- 遅いクエリを特定
EXPLAIN ANALYZE
SELECT * FROM evaluations
WHERE store_id = 'xxx'
ORDER BY created_at DESC
LIMIT 10;

-- 複合インデックス追加
CREATE INDEX idx_evaluations_store_date
ON evaluations(store_id, created_at DESC);

CREATE INDEX idx_conversations_user_scenario
ON conversations(user_id, scenario_id, created_at DESC);

-- パーティショニング（大量データ対策）
CREATE TABLE conversations_2025 PARTITION OF conversations
FOR VALUES FROM ('2025-01-01') TO ('2026-01-01');
```

#### タスク2: クエリ最適化

```python
# app.py - N+1問題の解消

# ❌ 悪い例（N+1問題）
conversations = supabase.table('conversations').select('*').execute()
for conv in conversations.data:
    evaluation = supabase.table('evaluations').select('*').eq('conversation_id', conv['id']).execute()

# ✅ 良い例（JOIN）
conversations = supabase.table('conversations')\
    .select('*, evaluations(*)')\
    .execute()
```

#### チェックリスト

- [ ] インデックス追加完了
- [ ] クエリ実行時間測定（<100ms目標）
- [ ] N+1問題解消確認

---

### Day 3: キャッシュ戦略

#### タスク1: Redis導入

```bash
# Redisインストール（開発環境）
brew install redis
redis-server

# Python依存関係
pip install redis
```

```python
# app.py
import redis
import json

# Redis接続
redis_client = redis.Redis(
    host='localhost',
    port=6379,
    decode_responses=True
)

# RAG検索結果をキャッシュ
def search_rag_with_cache(query: str, scenario_id: str, top_k: int = 3):
    cache_key = f"rag:{scenario_id}:{query[:50]}"

    # キャッシュチェック
    cached = redis_client.get(cache_key)
    if cached:
        return json.loads(cached)

    # RAG検索実行
    results = search_rag_by_scenario(query, scenario_id, top_k)

    # キャッシュに保存（1時間）
    redis_client.setex(cache_key, 3600, json.dumps(results))

    return results

# シナリオデータをキャッシュ
def load_scenario_with_cache(scenario_id: str):
    cache_key = f"scenario:{scenario_id}"

    cached = redis_client.get(cache_key)
    if cached:
        return json.loads(cached)

    scenario = load_scenario_object(scenario_id)
    redis_client.setex(cache_key, 86400, json.dumps(scenario))  # 24時間

    return scenario
```

#### チェックリスト

- [ ] Redis統合完了
- [ ] キャッシュ動作確認
- [ ] ヒット率測定（70%以上目標）

---

### Day 4: フロントエンド最適化

#### タスク1: コード分割

```tsx
// src/App.tsx
import { lazy, Suspense } from 'react'

// 遅延ロード
const HistoryPage = lazy(() => import('@/components/History/HistoryPage'))
const AdminDashboard = lazy(() => import('@/components/Admin/AdminDashboard'))

export default function App() {
  return (
    <Suspense fallback={<LoadingSpinner />}>
      <Routes>
        <Route path="/history" element={<HistoryPage />} />
        <Route path="/admin" element={<AdminDashboard />} />
      </Routes>
    </Suspense>
  )
}
```

#### タスク2: ビルド最適化

```bash
# vite.config.ts
export default defineConfig({
  build: {
    rollupOptions: {
      output: {
        manualChunks: {
          'react-vendor': ['react', 'react-dom'],
          'chart-vendor': ['chart.js', 'react-chartjs-2'],
        }
      }
    },
    chunkSizeWarningLimit: 1000
  }
})

# ビルド実行
npm run build

# バンドルサイズ確認
npm run build -- --stats
```

#### チェックリスト

- [ ] コード分割実装
- [ ] バンドルサイズ測定（<500KB目標）
- [ ] Lighthouseスコア90以上

---

### Day 5: 負荷テスト

#### タスク1: Locustによる負荷テスト

```python
# locustfile.py
from locust import HttpUser, task, between

class RoleplayUser(HttpUser):
    wait_time = between(1, 5)

    def on_start(self):
        # ログイン
        self.client.post("/api/auth/login", json={
            "email": "test@example.com",
            "password": "test123"
        })

    @task(3)
    def send_message(self):
        self.client.post("/api/chat", json={
            "message": "SNS動画の予算はどれくらいですか?",
            "scenario_id": "meeting_1st",
            "history": []
        })

    @task(1)
    def get_evaluation(self):
        self.client.post("/api/evaluate", json={
            "messages": [
                {"speaker": "営業", "text": "こんにちは"},
                {"speaker": "顧客", "text": "こんにちは"}
            ],
            "scenario_id": "meeting_1st"
        })

    @task(1)
    def get_history(self):
        self.client.get("/api/conversations?user_id=xxx")
```

```bash
# 負荷テスト実行
locust -f locustfile.py --host=http://localhost:5000

# 100ユーザー同時接続でテスト
# 目標: レスポンスタイム<1秒、エラー率<1%
```

#### チェックリスト

- [ ] 負荷テスト実施
- [ ] ボトルネック特定・解消
- [ ] 100同時接続クリア

---

### Week 7まとめ

✅ 100店舗対応完了

**次週**: Week 8 - テスト・デプロイ

---

## 📅 Week 8: テスト・デプロイ

**目標**: 本番稼働可能

### Day 1-2: E2Eテスト

#### タスク1: Playwright統合

```bash
npm install -D @playwright/test
npx playwright install
```

```typescript
// tests/e2e/roleplay.spec.ts
import { test, expect } from '@playwright/test'

test.describe('ロープレフロー', () => {
  test('ログインから講評までの完全フロー', async ({ page }) => {
    // ログイン
    await page.goto('http://localhost:5173/login')
    await page.click('button:has-text("Googleでログイン")')

    // 会話開始
    await page.waitForSelector('text=1次面談')
    await page.click('button:has-text("会話開始")')

    // メッセージ送信
    await page.fill('textarea', 'SNS動画の予算はどれくらいですか?')
    await page.click('button:has-text("送信")')

    // AI応答待機
    await page.waitForSelector('.bot-message')
    const botMessage = await page.textContent('.bot-message')
    expect(botMessage).toBeTruthy()

    // 講評表示
    await page.click('button:has-text("講評を見る")')
    await page.waitForSelector('text=ヒアリング力')

    const score = await page.textContent('.average-score')
    expect(score).toMatch(/\d+点/)
  })

  test('シナリオ切り替え', async ({ page }) => {
    await page.goto('http://localhost:5173')

    // シナリオ選択
    await page.selectOption('select[name="scenario"]', 'meeting_2nd')

    // 確認ダイアログ
    page.on('dialog', dialog => dialog.accept())

    // シナリオが変更されたことを確認
    await expect(page.locator('text=2次面談')).toBeVisible()
  })
})
```

```bash
# テスト実行
npx playwright test

# ヘッドレスモードOFF（デバッグ用）
npx playwright test --headed
```

#### チェックリスト

- [ ] E2Eテスト10シナリオ以上作成
- [ ] 全テストパス
- [ ] CI/CD統合

---

### Day 3: デプロイ準備

#### タスク1: 環境変数管理

```bash
# .env.production（本番用）
VITE_SUPABASE_URL=https://xxx.supabase.co
VITE_SUPABASE_ANON_KEY=eyJhbGc...

SUPABASE_SERVICE_ROLE_KEY=eyJhbGc...（バックエンドのみ）

OPENAI_API_KEY=sk-...

REDIS_URL=redis://...
```

#### タスク2: Vercel設定（フロントエンド）

```bash
# Vercel CLIインストール
npm install -g vercel

# ログイン
vercel login

# デプロイ
vercel --prod

# 環境変数設定
vercel env add VITE_SUPABASE_URL
vercel env add VITE_SUPABASE_ANON_KEY
```

#### タスク3: Railway設定（バックエンド）

```yaml
# railway.toml
[build]
builder = "nixpacks"

[deploy]
startCommand = "gunicorn app:app"
healthcheckPath = "/health"
restartPolicyType = "on-failure"

[[services]]
name = "api"
port = 5000
```

```bash
# Railway CLIインストール
npm install -g @railway/cli

# ログイン
railway login

# プロジェクト作成
railway init

# デプロイ
railway up

# 環境変数設定
railway variables set OPENAI_API_KEY=sk-...
railway variables set SUPABASE_URL=https://...
```

#### チェックリスト

- [ ] 環境変数設定完了
- [ ] Vercelデプロイ成功
- [ ] Railwayデプロイ成功
- [ ] HTTPS設定完了

---

### Day 4: 本番テスト

#### タスク1: スモークテスト

```bash
# 本番環境での動作確認
1. ログイン動作確認
2. 6シナリオ全て動作確認
3. 会話保存確認
4. 評価生成確認
5. 履歴閲覧確認
6. CSV出力確認
7. 店舗管理機能確認
```

#### タスク2: セキュリティチェック

```bash
# SSL/TLS確認
curl -I https://your-domain.com

# CORS設定確認
# Headers確認
curl -H "Origin: https://evil.com" https://api.your-domain.com

# RLS動作確認
# 他ユーザーのデータが見えないことを確認
```

#### チェックリスト

- [ ] 全機能動作確認
- [ ] セキュリティチェック完了
- [ ] パフォーマンステスト合格

---

### Day 5: リリース・ドキュメント整備

#### タスク1: ユーザーマニュアル作成

```markdown
# SNS動画営業ロープレシステム ユーザーマニュアル

## 1. ログイン方法
1. https://your-app.vercel.app にアクセス
2. 「Googleでログイン」をクリック
3. 店舗コード（STORE_001など）を入力
4. 名前を入力して登録完了

## 2. ロープレ実施方法
1. ヘッダーからシナリオを選択
2. 「会話開始」をクリック
3. 音声またはテキストで会話
4. 「講評を見る」をクリックして評価確認

## 3. 履歴閲覧
1. 「練習履歴」タブをクリック
2. 過去のロープレ一覧を確認
3. 「詳細を見る」で会話内容を確認

## 4. CSV出力
1. 練習履歴ページで「CSV出力」をクリック
2. ダウンロードされたCSVをExcelで開く
```

#### タスク2: 管理者マニュアル作成

```markdown
# 管理者マニュアル

## 店舗コード発行
1. 管理画面にログイン
2. 「店舗管理」をクリック
3. 店舗情報を入力して「追加」

## 統計確認
1. 「ダッシュボード」で全店舗の統計を確認
2. 「ランキング」で店舗別順位を確認

## ユーザー管理
1. 「ユーザー管理」で全ユーザーを確認
2. 権限変更・削除が可能
```

#### タスク3: リリースノート作成

```markdown
# v1.0.0 リリースノート

## 🎉 新機能
- Google認証 + 店舗コード認証
- 6シナリオ対応（1次面談〜追加営業）
- RAG搭載（600-800件の実例データ）
- AI評価機能（GPT-4）
- 練習履歴・成長グラフ
- CSV出力
- 店舗管理・ランキング機能
- 100店舗対応

## 🔧 技術スタック
- Frontend: React + TypeScript + Vite
- Backend: Flask + Python
- Database: Supabase (PostgreSQL)
- AI: OpenAI GPT-4o-mini / GPT-4
- RAG: FAISS + text-embedding-3-large
- Deploy: Vercel + Railway

## 📊 パフォーマンス
- 応答速度: <1秒
- 同時接続: 100ユーザー対応
- 稼働率: 99.9%目標

## 📝 制限事項
- 音声入力: 1回30秒まで
- 会話履歴: 最大50ターン
- CSV出力: 最大1000件
```

#### チェックリスト

- [ ] ユーザーマニュアル作成
- [ ] 管理者マニュアル作成
- [ ] リリースノート作成
- [ ] 本番リリース完了

---

### Week 8まとめ

✅ 本番稼働可能

---

## 📊 進捗管理シート

```
現在地: Week 1 完了 → Week 2 開始

全体進捗: ■■■■■■■■□□□□□□□□□□□□□□□□ 35%

Week 1: ■■■■■■■ ✅ 完了（2025-11-11）
Week 2: □□□□□□□ ⏳ 次のステップ
Week 3: □□□□□
Week 4: □□□□□
Week 5: □□□□□
Week 6: □□□□□
Week 7: □□□□□
Week 8: □□□□□
```

---

## ✅ 次のアクション

### ✅ Week 1 完了（2025-11-11）

**完了した内容:**
- ✅ Supabaseプロジェクト作成完了
- ✅ Google OAuth設定完了
- ✅ データベース設計・構築完了
- ✅ RLS設定完了
- ✅ 認証フロー実装完了
- ✅ 店舗管理画面実装完了
- ✅ ダークテーマUI統一完了

### 🎯 次のステップ: Week 2（6シナリオ構築 + RAG構築）

**今すぐ始めること:**

1. **6シナリオのJSON作成（Day 1-2）**
   ```bash
   # scenariosディレクトリ作成
   mkdir -p scenarios
   cd scenarios

   # 6つのシナリオファイルを作成
   touch meeting_1st.json      # 1次面談
   touch meeting_1_5.json      # 1.5次面談
   touch meeting_2nd.json      # 2次面談
   touch meeting_3rd.json      # 3次面談
   touch kickoff.json          # キックオフMTG
   touch upsell.json           # 追加営業
   touch index.json            # シナリオ一覧
   ```

2. **音声データの準備（Day 3-4）**
   - 無料相談の録音30本を準備
   - Whisper APIで文字起こし
   - 営業・相談者の話者分離
   - シナリオ別にタグ付け

3. **RAGデータ準備（Day 3-4）**
   - 各シナリオ100-150件のQ&A例を作成
   - 合計600-800件のデータベース構築

**Week 2の目標:**
- 6シナリオの詳細設計完了
- RAGデータベース600-800件作成
- シナリオ別検索機能実装

**Week 2完了後の成果物:**
- ✅ 6シナリオJSON（ペルソナ・ガイドライン含む）
- ✅ RAGインデックス構築済み
- ✅ シナリオ別検索機能動作確認

### 開発開始チェックリスト（Week 2向け）

- [x] Supabaseプロジェクト作成完了
- [x] Google OAuth設定完了
- [x] 開発環境セットアップ完了
- [ ] 音声データ30本準備完了
- [ ] シナリオ設計ドキュメント確認
- [ ] Week 2 Day 1開始

---

*この手順書はSNS動画営業ロープレシステムの開発を段階的に進めるためのガイドです。*
*最終更新: 2025年11月11日（Week 1完了）*

---

## 🎉 2025年12月5日 更新: ChatGPT風音声会話・RAG検索有効化

### 実装完了内容

#### 1. ChatGPT風の音声会話機能（VAD）
- **VAD（音声自動検出）**: マイクボタン1つでハンズフリー会話
- **音声検出閾値**: 75（環境音誤検出対策）
- **音声継続時間**: 400ms（瞬間的なノイズ除外）
- **最低録音時間**: 1000ms（短すぎる録音をスキップ）
- **割り込み機能**: AI話し中でも割り込み可能（閾値85）

実装ファイル:
- `src/lib/audio.ts`: VADエンジン
- `src/RoleplayApp.tsx`: VAD制御とストリーミング

#### 2. RAG検索の有効化
- **データ件数**: 898件の実際のロープレ会話パターン
- **シナリオ別フィルタリング**: 
  - meeting_1st: 324件
  - upsell: 231件  
  - meeting_3rd: 128件
  - kickoff: 113件
  - meeting_2nd: 102件
- **速度最適化**: top_k=2, 最大200文字, 直近3件の会話のみ
- **検索時間**: 約100-150ms（Embedding + FAISS検索）

実装ファイル:
- `app.py:704-727`: RAG検索ロジック
- `rag_index/sales_patterns.faiss`: FAISSインデックス（11MB）
- `rag_index/sales_patterns.json`: メタデータ（414KB）

#### 3. 字幕とアバターの完全同期
- 音声再生と同時に対応する字幕を表示
- 音声終了時に字幕をクリア
- 2行制限でモバイル対応

実装ファイル:
- `src/RoleplayApp.tsx:198-224`: 音声・字幕同期
- `src/components/MediaPanel.tsx:93`: 字幕表示（line-clamp-2）

#### 4. UI/UX改善
- マイクボタン1つに統合（187行削除）
- 音声初期化メッセージを削除
- アバター固定表示（PC画面でスクロールしない）

実装ファイル:
- `src/components/Composer.tsx`: シンプル化されたUI
- `src/RoleplayApp.tsx:759`: sticky positioning

#### 5. レスポンス速度改善
- 無音検出: 1000ms → 700ms
- TTSモデル: tts-1-hd → tts-1（50%高速化）
- 約1秒のレスポンス改善

実装ファイル:
- `src/lib/audio.ts:31`: 無音検出時間
- `app.py`: tts-1モデル使用

### Git コミット履歴（過去2週間）

```
d60f298 - feat: RAG検索を有効化して898件のロープレデータを活用
b2c66e4 - fix: VAD閾値をさらに引き上げて環境音の誤検出を防止
e14761f - fix: 音声初期化メッセージ削除と字幕同期の改善
72dc0a7 - fix: VAD誤検出を防止し字幕表示を改善
e35cfdd - feat: マイクボタンを会話モードに統合
b3eacfc - feat: UI/UX改善（字幕・表情・音声有効化）
1e8e2da - perf: レスポンス速度を大幅に改善
cf5c00e - feat: 会話モード停止時にAI音声も即座に停止
cc7c666 - fix: VAD重複録音による同時会話を防止
0f93932 - fix: 割り込み時の音声停止を強化
1377e82 - fix: useRefでVADモード状態を管理してクロージャー問題を完全に解決
ed84d49 - feat: ChatGPT風の割り込み機能を実装
d4afc39 - fix: ペルソナのランダム選択を削除し一貫性を確保
2f1fca9 - fix: PC画面でアバターが下に押し出される問題を修正
```

### 技術的な改善点

#### React クロージャー問題の解決
```typescript
// 問題: 関数が古い状態値をキャプチャ
const [isVADMode, setIsVADMode] = useState(false);

// 解決: useRefで最新値を保持
const isVADModeRef = useRef(false);
const isSendingRef = useRef(false);
```

#### SSEストリーミングの割り込み対応
```typescript
// 音声停止 + SSEキャンセル
if (streamReader) {
  streamReader.cancel();
}
if (currentAudio) {
  currentAudio.pause();
  currentAudio.onended = null;
}
```

#### RAG検索のシナリオフィルタリング
```python
# シナリオ別にデータをフィルタリング
filtered_indices = [i for i, m in enumerate(RAG_METADATA) 
                   if m.get('scenario_id') == scenario_id]
```

### 今後の改善案

1. **RAG検索の高度化**
   - より多様な会話パターンの追加
   - シナリオ間の遷移パターンの学習

2. **VADの精度向上**
   - ユーザーごとの閾値カスタマイズ
   - 環境ノイズの自動検出

3. **字幕表示の強化**
   - リアルタイムでの分かち書き
   - 重要キーワードのハイライト

---

**更新日**: 2025年12月5日  
**更新者**: AI Assistant (Claude Code)
