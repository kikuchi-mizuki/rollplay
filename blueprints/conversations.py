    ...
    try:
        ...
        def generate():
            ...
            try:
                ...
                print("[DEBUG-GENERATE] GPT-4o-mini呼び出し開始（max_tokens=100、最適化設定）", flush=True)
                logger.info("[ストリーミング開始] GPT-4o-mini応答生成開始（max_tokens=100、最適化設定）")
                response = openai_client.chat.completions.create(
                    model="gpt-4o-mini",    # 高速モデル（会話のテンポ重視）
                    messages=messages,
                    max_tokens=100,         # 最適化設定: 2-4文の完結した応答（文章途切れ完全防止）
...
- クロージング力: 次のアクション・決定を促す適切なクロージング"""
        ...
        【必須事項】
        - **strengths（良かった点）**: 必ず3〜5項目を記載してください（空欄は絶対に不可）
          - 挨拶、言葉遣い、質問の仕方、声のトーンなど、小さな良い点でも必ず含める
          - 具体的な発言を引用し、「なぜ良いか」を明記
        - **improvements（改善点）**: 必ず3〜5項目を記載
          - 具体的な発言を引用し、「なぜ改善が必要か」「どう改善すべきか」を明記
