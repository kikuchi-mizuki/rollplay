...
- クロージング力: 次のアクション・決定を促す適切なクロージング"""
        ...
        {{
            ...
            "overall": "総合評価（全体の印象、優れていた点の総括、改善すべき点の総括、次回への具体的なアドバイス。200-400文字程度で詳しく記載）",
            "detailedFeedback": {{
                "hearing": {{
            ...
            "actionPlan": [
                "次回のロープレで実践すべき具体的なアクション1（50-80文字程度で詳しく）",
                "次回のロープレで実践すべき具体的なアクション2（50-80文字程度で詳しく）",
                "次回のロープレで実践すべき具体的なアクション3（50-80文字程度で詳しく）",
                "次回のロープレで実践すべき具体的なアクション4（50-80文字程度で詳しく）",
                "次回のロープレで実践すべき具体的なアクション5（50-80文字程度で詳しく）"
            ],
            "analysis": {{
                "hearing_count": 数値,
                "planning_count": 数値,
...
- 各項目の分析は80-150文字程度で詳しく記載してください
- rationaleは100-150文字程度で、良かった点と改善点の両方に触れながら詳細に分析してください
- 具体的な発言を多く引用し、なぜその発言が良い/改善が必要かを詳しく説明してください

【必須】良かった点（strengths）を必ず5-7個見つけて記載してください。小さな良い点でも評価対象です。"""
        else:
            system_message = """【最重要指示】あなたの出力は必ず有効なJSON形式のみとしてください。説明文や前置きは一切不要です。
...
【必須】良かった点（strengths）を必ず5-7個見つけて記載してください。小さな良い点でも評価対象です。"""
        ...
        try:
            response = openai_client.chat.completions.create(
                ...
                max_tokens=5000,  # 30分のロールプレイに対応した詳細な講評のためトークン数を大幅に増量
                temperature=0.3
            )
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            if 'actionPlan' not in evaluation or not evaluation['actionPlan']:
                ...
                evaluation['actionPlan'] = [
                    ...
                    "会話全体の流れを意識し、挨拶→ヒアリング→提案→クロージングの各段階を丁寧に進めましょう"
                ]
